...
class db {
  ...
  addWords(words, groupId, action) {
    const addWordsQuery = `
		INSERT INTO words (word, group_id, action)
		VALUES 
  ...
  checkWordsExist(words, groupId) {
    return new Promise((resolve, reject) => {
      this.all(
        `SELECT word FROM WORDS WHERE (${words
          .map((word) => `word = '${word}'`)
          .join(" OR ")}) AND group_id = '${groupId}'`
      ).then((rows) => {
  ...
  deleteWordsByGroup(groupId) {
    return this.run(`DELETE FROM words WHERE group_id = '${groupId}'`);
  }
